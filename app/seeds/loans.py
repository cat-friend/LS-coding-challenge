...
def seed_loans():
    ...
    lengths_month = [10.5, 28.3, 31, 14.5, 5.9, 16, 27]
    monthly_payments = [10, 200, 3000, 4400, 50, 665, 7000]
    for i in range(0, 7):
        loan = Loan(amount=amounts[i], interest_rate=interest_rates[i], length_month=lengths_month[i], monthly_payment=monthly_payments[i])
        db.session.add(loan)
